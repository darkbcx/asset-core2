 ...
 */
export async function createUser(userData: CreateUser, passwordHash: string): Promise<User> {
  const userId = uuidv4();
  const now = new Date();
...
export async function updateUser(userId: string, userData: UpdateUser): Promise<BackendResponse<User>> {
  ...
  if (validatedData.system_permissions !== undefined) {
    ...
    params.push(validatedData.system_permissions ? JSON.stringify(validatedData.system_permissions) : null);
  }
 ...
 */
export async function getActiveUsersCountByCompany(companyId: string): Promise<number> {
  const query = `
 ...
 */
export async function createUserCompany(userCompanyData: CreateUserCompany): Promise<UserCompany> {
  // Validate input
 ...
 */
export async function getUserCompaniesByUserId(userId: string, includeInactive = false): Promise<UserCompany[]> {
  let query = `
 ...
 */
export async function getUserCompaniesWithDetails(userId: string): Promise<Array<UserCompany & { company_name: string; company_slug: string }>> {
  const query = `
    SELECT 
      uc.*,
  ...
  return rows as Array<UserCompany & { company_name: string; company_slug: string }>;
}
 ...
 */
export async function updateUserCompany(associationId: string, userCompanyData: UpdateUserCompany): Promise<UserCompany> {
  // Validate input
 ...
 */
export async function getUserCompanyById(associationId: string): Promise<UserCompany | null> {
 ...
 */
export async function removeUserFromCompany(associationId: string): Promise<boolean> {
 ...
 */
export async function setPrimaryCompany(userId: string, companyId: string): Promise<UserCompany> {
  // Get user companies
  const companies = await getUserCompaniesByUserId(userId);
  
  // Find the target association
  const targetAssociation = companies.find(uc => uc.company_id === companyId && uc.is_active);
  ...
  for (const uc of companies.filter(c => c.is_primary && c.id !== targetAssociation.id)) {
    await updateUserCompany(uc.id, { is_primary: false });
  }
  
  // Set the target association as primary
  const updated = await updateUserCompany(targetAssociation.id, { is_primary: true });
...
export async function userHasCompanyAccess(userId: string, companyId: string): Promise<boolean> {
  ...
  const [rows] = await db.execute<{ count: number }>(query, [userId, companyId]);
